...
function locateSegmentBreakpoint(segment, splitMoveDistance, { mp, gridless, numPrevDiagonal } = {}) {
  ...
  if ( !gridless ) {
    ...
    if ( breakpoint.equals(A) ) breakpoint.z = A.z;
      ...
      const z = CONFIG.GeometryLib.utils.gridUnitsToPixels(elevation) * 2;
...
function _splitSegmentAt(segment, breakpoint, mp, numPrevDiagonal = 0) {
  ...
  // Split the calculated path if it exists
  if (segment._calculatedPath && segment._calculatedPath.length > 0) {
    // Find the closest point in the path to the breakpoint
    const breakpointGridPos = canvas.grid.getOffset(breakpoint);
    let closestIndex = 0;
    let minDist = Infinity;

    for (let i = 0; i < segment._calculatedPath.length; i++) {
      const pathPoint = segment._calculatedPath[i];
      const dist = Math.abs(pathPoint.i - breakpointGridPos.i) + Math.abs(pathPoint.j - breakpointGridPos.j);
      if (dist < minDist) {
        minDist = dist;
        closestIndex = i;
      }
    ...
    // Split the path at the closest index
    s0._calculatedPath = segment._calculatedPath.slice(0, closestIndex + 1);
    s1._calculatedPath = segment._calculatedPath.slice(closestIndex);
  }
