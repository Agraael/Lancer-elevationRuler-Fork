...
/**
 * Get the size of a hex token (based on drag ruler implementation)
 * @param {Token} token
 * @returns {number} Size of the token (1-5)
 */
function getHexTokenSize(token) {
...
/**
 * Check if token uses alternative orientation for hexes
 * @param {Token} token
 * @param {number} size - Token size
 * @returns {boolean}
 */
function getAltOrientationFlagForToken(token, size) {
...
/**
 * Get the shape (grid spaces occupied) of a token
 * Based on drag ruler implementation
 * @param {Token} token
 * @returns {Array<{x: number, y: number}>} Array of relative grid offsets
 */
export function getTokenShape(token) {
  ...
  // Gridless - token is at a single point
  if (scene.grid.type === CONST.GRID_TYPES.GRIDLESS) {
    return [{x: 0, y: 0}];
  }

  // Square grid - calculate all occupied squares
...
/**
 * Get the area occupied by a token shape at a given position, accounting for hex row shifting
 * Based on drag ruler implementation
 * @param {object} position - Grid position {x, y} where x=column, y=row
 * @param {Array<{x: number, y: number}>} shape - Token shape as relative offsets
 * @returns {Array<{x: number, y: number}>} Array of absolute grid positions
 */
export function getAreaFromPositionAndShape(position, shape) {
  const isHex = canvas.grid.type !== CONST.GRID_TYPES.SQUARE && canvas.grid.type !== CONST.GRID_TYPES.GRIDLESS;

  return shape.map(space => {
    let x = position.x + space.x;
    let y = position.y + space.y;

    // Handle hex grid row shifting
    if (isHex) {
      let shiftedRow;
      ...
      if (canvas.grid.grid.columnar) {
        ...
        // Row-based hex grid (horizontal rows)
        // Same logic as drag ruler
