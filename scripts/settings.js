...
export class Settings extends ModuleSettingsAbstract {
   ...
   */
    static registerAll() {
        ...
        register(KEYS.TERRAIN_HEIGHT_TOOLS, {
            name: localize(`${KEYS.TERRAIN_HEIGHT_TOOLS}.name`),
            hint: localize(`${KEYS.TERRAIN_HEIGHT_TOOLS}.hint`),
            scope: "world",
            config: true,
            default: true,
            type: Boolean,
    ...
    static registerKeybindings() {
        ...
        game.keybindings.register(MODULE_ID, KEYBINDINGS.ADD_WAYPOINT, {
            ...
            onDown: context => {
                if (canvas.controls?.ruler && !canvas.controls.ruler._isTokenRuler)
                    toggleTokenRulerWaypoint(context, true);
            },
            precedence: CONST.KEYBINDING_PRECEDENCE.NORMAL
        });
        ...
        game.keybindings.register(MODULE_ID, KEYBINDINGS.REMOVE_WAYPOINT, {
            ...
            onDown: context => {
                if (canvas.controls?.ruler && !canvas.controls.ruler._isTokenRuler)
                    toggleTokenRulerWaypoint(context, false);
            },
            precedence: CONST.KEYBINDING_PRECEDENCE.NORMAL
        });
        ...
        game.keybindings.register(MODULE_ID, KEYBINDINGS.TOKEN_RULER.ADD_WAYPOINT, {
            ...
            onDown: context => {
                if (canvas.controls?.ruler._isTokenRuler)
                    toggleTokenRulerWaypoint(context, true);
            },
            precedence: CONST.KEYBINDING_PRECEDENCE.NORMAL
        });
        ...
        game.keybindings.register(MODULE_ID, KEYBINDINGS.FREE_MOVEMENT, {
            name: game.i18n.localize(`${MODULE_ID}.keybindings.${KEYBINDINGS.FREE_MOVEMENT}.name`),
            hint: game.i18n.localize(`${MODULE_ID}.keybindings.${KEYBINDINGS.FREE_MOVEMENT}.hint`),
            editable: [
                { key: "KeyF" }
            ],
            onDown: () => {
   ...
   */
    static useSpeedHighlighting(token) {
      ...
      || (choice === SH.CHOICES.COMBAT && !game.combat?.started))
            return false;
        if (game.user.isGM || !this.get(SH.NO_HOSTILES))
            return true;
    ...
    static useAutoMoveDetection() {
        return game.system.id === "dnd5e" && this.get(this.KEYS.MEASURING.AUTO_MOVEMENT_TYPE);
    }
}
...
function toggleTokenRulerWaypoint(context, add = true) {
    ...
    const ruler = canvas.controls.ruler;
    if (!canvas.tokens.active || !ruler || !ruler.active)
        return;
    ...
    if (delta < 100)
        return true; // Throttle keyboard movement once per 100ms
    MOVE_TIME = now;

    log(`${add ? "adding" : "removing"}TokenRulerWaypoint`);
    if (add)
        ruler._addWaypoint(position);
    else if (ruler.waypoints.length > 1)
        ruler._removeWaypoint(position); // Removing the last waypoint throws errors.
}
